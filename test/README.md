...
Native projects live in test/native/current and test/native/minimum with shared assertions in test/native/shared. CI runs the shared current native fixture on Android and iOS. The workflow_dispatch native_profile input also exposes exploratory legacy runs; they are not routine compatibility checks. Current RN 0.87.1 pairs with React 19.2.3. Minimum RN 0.59.10 pairs with React 16.8.3 and remains exploratory: the declared RN peer range rejects it. A forced installation or a web pass is not legacy native compatibility evidence.

To run the minimum Android probe locally, prepare the minimum fixture and candidate packages, start one Android emulator, install Maestro, and make Java 17 the default while keeping Java 8 available for Gradle 5.4.1. Then run `NODE_OPTIONS=--openssl-legacy-provider node test/integration/native/run-platform.ts --platform android --fixture .tmp/native/fixtures/minimum --android-gradle-java-home "$JAVA_HOME_8_X64"`. The runner bundles JS into the app's Android assets, builds and installs on the online adb serial, then runs Maestro against that same serial. The CI `minimum-android` profile performs this probe without running iOS.
