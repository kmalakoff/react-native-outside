...
async function bundleAssert(profile, output) {
  ...
  await esbuild.build({
    ...
    absWorkingDir: root,
    bundle: true,
    // Browser tests do not have Node's process global; disable these Node-only checks.
    define: { global: 'globalThis', 'process.env.NODE_DEBUG': '""', 'process.emitWarning': 'undefined' },
    format: 'esm',
    logLevel: 'silent',
    outfile: path.join(root, output),
    platform: 'browser',
